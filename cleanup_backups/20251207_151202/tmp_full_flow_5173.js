...
function postLogin(cb) {
  const opts = { host: 'localhost', port: 5173, path: '/api/auth/login', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(data) } };
...
function getMe(token, cb) {
  const opts = { host: 'localhost', port: 5173, path: '/api/auth/me', method: 'GET', headers: { 'Authorization': `Bearer ${token}` } };
  const req = http.request(opts, res => {
    let body = '';
    res.on('data', d => body += d);
